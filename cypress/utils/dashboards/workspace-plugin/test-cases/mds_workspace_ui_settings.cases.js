        ...
        it('CRUD operations inside application settings should work as expected', () => {
          ...
          cy.get(switchSelector).then(($switch) => {
            if ($switch.prop('disabled')) {
              ...
              cy.get(switchSelector).click({ force: true });

              // Check if save button exists (it may not exist in some environments)
              cy.get('body').then(($body) => {
                ...
                if (saveBtn.length > 0 && !saveBtn.prop('disabled')) {
                  ...
                  cy.log(
                    'Save button not found or disabled, skipping save operation.'
                  );
                }
              });
            }
