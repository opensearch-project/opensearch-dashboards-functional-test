...
describe('Snapshots', () => {
  ...
  describe('Repository can be created', () => {
    it('successfully creates a new repository', () => {
      ...
      cy.get(`input[placeholder="e.g., /mnt/snapshots"]`)
        .focus()
        .type('~/Desktop');
  ...
  describe('Snapshot can be created', () => {
    it('successfully creates a new snapshot', () => {
      ...
      cy.get(`input[data-test-subj="snapshotNameInput"]`).type(
        'test_snapshot{enter}'
      );

      // Select all indexes to be included
      cy.get(`[data-test-subj="indicesComboBoxInput"]`).type(
        'test_index_1{enter}'
      );
      cy.get(`[data-test-subj="indicesComboBoxInput"]`).type(
        'test_index_2{enter}'
  ...
  describe('Snapshot can be restored', () => {
    it('Successfully restores indices from snapshot', () => {
      ...
      cy.get(
        `[data-test-subj="checkboxSelectRow-test_repo:test_snapshot"]`
      ).check({ force: true });
