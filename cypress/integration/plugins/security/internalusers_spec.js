...
describe('Internal users page', () => {
  ...
  it('should create new user successfully', () => {
    ...
    cy.get('input[data-test-subj="name-text"]').type(userName, { force: true });
    cy.get('input[data-test-subj="name-text"]').should('have.value', userName);


    cy.get('input[data-test-subj="password"]').type(password, { force: true });
    cy.get('input[data-test-subj="re-enter-password"]').type(password, { force: true });


    cy.mockInternalUsersAction(
      SEC_INTERNALUSERS_FIXTURES_PATH + '/internalusers_response_post_new_user_creation.json',
      () => {
