...
describe('Roles page', () => {
  ...
  it('should open Cluster Permissions drop-down menu when clicked', () => {
    ...
    cy.contains('span', 'cluster:admin/opendistro/ad/detector/search').should('not.exist');
  ...
  it.only('should create new role successfully', () => {
    ...
    cy.get('input[data-test-subj="name-text"]').type(roleName, { force: true });

    cy.get('input[data-test-subj="name-text"]').should('have.value',roleName);
