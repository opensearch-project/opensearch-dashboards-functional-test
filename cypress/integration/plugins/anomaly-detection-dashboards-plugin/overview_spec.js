...
context('Overview page', () => {
  ...
  it.skip('Empty dashboard redirects to overview page', () => {
    cy.visit(AD_URL.DASHBOARD);
    cy.getElementByTestId('sampleDetectorButton').click();
    validatePageElements();
  });

  it.skip('Empty detector list redirects to overview page', () => {
    cy.visit(AD_URL.DETECTOR_LIST);
    cy.getElementByTestId('sampleDetectorButton').click();
    validatePageElements();
  });

  it.skip('Side nav AD button redirects to overview page', () => {
    cy.visit(AD_URL.DETECTOR_LIST);
  ...
  context('Flyouts open and close', () => {
    it.skip('HTTP responses sample detector', () => {
      cy.visit(AD_URL.OVERVIEW);
      validatePageElements();
      openAndCloseFlyout(0);
    });

    it.skip('eCommerce sample detector', () => {
      cy.visit(AD_URL.OVERVIEW);
      validatePageElements();
      openAndCloseFlyout(1);
    });

    it.skip('Host health sample detector', () => {
      cy.visit(AD_URL.OVERVIEW);
      validatePageElements();
