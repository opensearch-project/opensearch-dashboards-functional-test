...
describe('Testing services table', () => {
  ...
  it('Verify columns in Services table', () => {
    cy.get('.euiFlexItem.euiFlexItem--flexGrow10 .panel-title')
      .contains('Services')
      .should('exist');
    cy.get('.euiTableCellContent__text[title="Name"]').should('exist');
    cy.get('.euiTableCellContent__text[title="Average latency (ms)"]').should(
      'exist'
    );
    ...
    cy.get(
      '.euiTableCellContent__text[title="No. of connected services"]'
    ).should('exist');
    cy.get('.euiTableCellContent__text[title="Connected services"]').should(
      'exist'
    );
  ...
  it('Navigate from Services to Traces', () => {
    ...
    cy.get('.euiBadge__childButton[data-test-subj="filterBadge"]').should(
      'exist'
    );
  });
});
...
describe('Testing traces Spans table verify table headers functionality', () => {
  ...
  it('Renders the spans table and verify columns headers', () => {
    ...
    cy.get('.euiLink.euiLink--primary')
  ...
  it('Toggle columns and verify the columns hidden text verify rows', () => {
    ...
    cy.get(
      '.euiSwitch.euiSwitch--compressed.euiSwitch--mini .euiSwitch__button'
    )
      .eq(3)
      .click();
  ...
  it('Render Spans table and and click on sort', () => {
    ...
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-spanId'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-parentSpanId"]'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-traceId"]'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-traceGroup'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-durationInNanos"]'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-startTime"]'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-endTime'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection-status.code"]'
    ).click();
    cy.get(
      '[data-test-subj="dataGridColumnSortingPopoverColumnSelection"]'
    ).click();
    cy.get('[data-test-subj="dataGridColumnSortingButton"]').should(
      'have.text',
      '8 fields sorted'
    );
    cy.get('[data-test-subj="dataGridColumnSortingButton"]')
...
describe('Testing traces Spans table and verify columns functionality', () => {
  ...
  it('Renders the spans table and click on first span to verify details', () => {
    ...
    cy.get('[data-test-subj="spanDetailFlyout"] .euiTitle.euiTitle--medium')
      .contains('Span detail')
      .should('exist');
    cy.get('.euiFlyoutBody .panel-title').contains('Overview').should('exist');
    cy.get('.euiTextColor.euiTextColor--subdued')
      .contains('Span ID')
      .should('exist');
    cy.get('[data-test-subj="parentSpanId"]')
      .contains('d03fecfa0f55b77c')
      .should('exist');
    cy.get('.euiFlyoutBody__overflowContent .panel-title')
      .contains('Span attributes')
      .should('exist');
    cy.get('.euiDescriptionList__description .euiFlexItem')
      .eq(0)
      .trigger('mouseover')
      .click();
...
describe('Testing switch mode to jaeger', () => {
  ...
  it('Verifies traces links to traces page with filter applied', () => {
    ...
    cy.get("[data-test-subj='filterBadge']")
      .eq(0)
      .contains('process.serviceName: customer');
  });
});
