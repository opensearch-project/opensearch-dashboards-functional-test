...
describe('Open flyout for a data row to see details', () => {
  ...
  it('Should be able to open flyout and see data, json and traces', () => {
    cy.get('[data-test-subj="docTable"] tbody tr button.euiButtonIcon')
      .first()
      .click();
  ...
  it('Close a tab', () => {
    ...
    cy.get('[data-test-subj="eventExplorer__topLevelTabbing"]')
      ...
      .then((lists) => {
        const initialLength = Cypress.$(lists).length;
        cy.get(
          '[data-test-subj="eventExplorer__topLevelTabbing"] button.euiTab'
        )
          .first()
    ...
    cy.get('[data-test-subj="eventExplorer__topLevelTabbing"]')
      ...
      .then((lists) => {
        ...
        cy.get(
          ...
          .click();
        cy.get(
          'button.euiTab-isSelected [data-test-subj="eventExplorer__tabClose"]'
        ).click();
        cy.get('[data-test-subj="eventExplorer__topLevelTabbing"]')
          .find('button.euiTab')
          .should('have.length', initialLength - 1);
...
describe('Toggle sidebar fields', () => {
  it('Toggle fields between available and selected section', () => {
    ...
    cy.get('[data-test-subj="docTable"]')
      .find('th')
      .contains('_source')
      .should('not.exist');
    cy.get('[data-test-subj="fieldToggle-AvgTicketPrice"]').click();
...
describe('Renders bar charts', () => {
  ...
  it('Renders vertical bar chart', () => {
    ...
    cy.wait(delayTime * 2);
    cy.get('[data-test-subj="comboBoxOptionsList "] span')
      .contains('Bar')
      .click();
    ...
    cy.get('[data-test-subj="comboBoxOptionsList "] button span')
      .contains('count()')
      .click();
    cy.get('#configPanel__chart_options [data-test-subj="comboBoxInput"]')
      .first()
      .click();
    cy.wait(delayTime);
    cy.get('[data-test-subj="comboBoxOptionsList "] button span')
...
describe('Renders line charts', () => {
  ...
  it('Renders line chart with threshold', () => {
    ...
    cy.get('[data-test-subj="comboBoxOptionsList "] button span')
      ...
      .click();
    cy.get(
      '#configPanel__chart_options [data-test-subj="comboBoxInput"]'
    ).click();
    cy.get('[data-test-subj="comboBoxOptionsList "] button span')
      .contains('Lines')
      .click();
    cy.get('#configPanel__Thresholds span')
      .contains('+ Add threadshold')
      .click();
    cy.get('[data-test-subj="nameFieldText"]').type('Max');
    cy.get('[data-test-subj="valueFieldNumber"]').type(3800);
    cy.get('[data-test-subj="visualizeEditorRenderButton"]').click();
    cy.wait(delayTime * 2);
    cy.get('g.text > g.textpoint text[data-unformatted|="Max"]').should(
      'exist'
    );
    cy.get(
      'g.xaxislayer-above > g.xtick text[data-unformatted|="artifacts.opensearch.org"]'
...
describe('Renders heatmap chart', () => {
  ...
  it('Renders heatmap chart with different z-axes', () => {
    ...
    cy.wait(delayTime * 2);
    cy.get(
      '#configPanel__value_options [data-test-subj="comboBoxInput"]'
    ).click();
    cy.wait(delayTime * 2);
    cy.get('[data-test-subj="comboBoxOptionsList "] button span')
      .contains('count()')
