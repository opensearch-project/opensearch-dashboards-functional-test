...
describe('Viewing application', () => {
  ...
  it('Saves visualization #2 to panel with availability level', () => {
    ...
    cy.get('[data-test-subj="app-analytics-logTab"]').click();
    cy.get('[data-test-subj="superDatePickerApplyTimeButton"]').click();
    cy.get('[id="explorerPlotComponent"]', { timeout: TIMEOUT_DELAY }).should('exist');
    cy.get('[data-test-subj="searchAutocompleteTextArea"]').focus();
...
describe('Separate from other plugins', () => {
  ...
  it('Hides application visualizations in Operational Panels', () => {
    ...
    cy.get('[data-test-subj="operationalPanelsActionsButton"]', { timeout: TIMEOUT_DELAY }).click();
    cy.get('[data-test-subj="addSampleContextMenuItem"]', { timeout: TIMEOUT_DELAY }).click();
    cy.get('[data-test-subj="confirmModalConfirmButton"]', { timeout: TIMEOUT_DELAY }).click();
  ...
  it('Hides application panels in Operational Panels', () => {
    ...
    cy.get('[data-test-subj="operationalPanelSearchBar"]', { timeout: TIMEOUT_DELAY }).type(`${nameOne}'s Panel`, {delay: TYPING_DELAY});
    cy.get('.euiTableCellContent__text').contains('No items found').should('exist');
    ...
    cy.get('[data-test-subj="operationalPanelsActionsButton"]', { timeout: TIMEOUT_DELAY }).click();
    cy.get('[data-test-subj="deleteContextMenuItem"]', { timeout: TIMEOUT_DELAY }).click();
    cy.get('[data-test-subj="popoverModal__deleteTextInput"]', { timeout: TIMEOUT_DELAY }).type('delete');
    cy.get('[data-test-subj="popoverModal__deleteButton"]', { timeout: TIMEOUT_DELAY }).should('not.be.disabled');
    cy.get('[data-test-subj="popoverModal__deleteButton"]', { timeout: TIMEOUT_DELAY }).click();
  });
});
