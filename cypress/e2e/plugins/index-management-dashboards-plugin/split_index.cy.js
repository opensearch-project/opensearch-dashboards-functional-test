...
describe('Split Index', () => {
  ...
  describe('can be created and updated', () => {
    ...
    it('Create an index successfully', () => {
      ...
      // type field name
      cy.get('[placeholder="Specify a name for the new index."]')
        .type(sampleIndex)
        .end();
      ...
      cy.get(`#_selection_column_${sampleIndex}-checkbox`)
        .should('have.exist')
        .end();

      cy.get(`[data-test-subj="viewIndexDetailButton-${sampleIndex}"]`)
        .click()
        .end();
      cy.get('#indexDetailModalSettings').click().end();

      cy.get(
        '[data-test-subj="form-name-index.number_of_shards"] .euiText'
      ).then(($shardNumber) => {
    ...
    it('Split successfully', () => {
      ...
      cy.get(`[data-test-subj="viewIndexDetailButton-${targetIndex}"]`)
        .click()
        .end();
      cy.get('#indexDetailModalSettings').click().end();
      cy.get('[data-test-subj="form-name-index.number_of_shards"] .euiText')
        .should('have.text', `${splitNumber}`)
        .end();
      cy.get('[data-test-subj="form-name-index.number_of_replicas"] input')
        .should('have.value', `${replicaNumber}`)
        .end();
    }); // Split
    ...
    it('Split successfully with advanced setting', () => {
      ...
      cy.get(`[data-test-subj="checkboxSelectRow-${sampleIndex}"]`)
        ...
        .clear()
        .type(
          `{"index.number_of_shards": "${splitNumber}", "index.number_of_replicas": "${replicaNumber}"}`,
          {
    ...
    it('Split successfully with alias', () => {
      ...
      cy.get(`[data-test-subj="checkboxSelectRow-${sampleIndex}"]`)
        ...
        .get(
          '[data-test-subj="form-name-aliases"] [data-test-subj="comboBoxSearchInput"]'
        )
