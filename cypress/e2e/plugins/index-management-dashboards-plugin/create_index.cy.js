  ...
  describe('can be created and updated', () => {
    ...
    it('Create a index successfully', () => {
      ...
      cy.get('[data-test-subj="comboBoxSearchInput"]')
        .get('[title="alias_for_specific_1"]')
        .should('exist');

      cy.get('[data-test-subj="comboBoxSearchInput"]').type(
        'some_test_alias{enter}'
      );
      ...
      cy.get('[data-test-subj="mapping-visual-editor-1-field-name"]').type(
        'text_mappings'
      );
      ...
      cy.get(`#_selection_column_${SAMPLE_INDEX}-checkbox`).should(
        'have.exist'
      );

      // check the index detail
      cy.visit(
        `${BASE_PATH}/app/${IM_PLUGIN_NAME}#/create-index/${SAMPLE_INDEX}`
      );
    ...
    it('Update settings successfully', () => {
      ...
      cy.wait(1000)
        .get('[data-test-subj="form-name-index.number_of_replicas"] input')
        .should('have.value', '2');
    });
    ...
    it('Update mappings successfully', () => {
      ...
      cy.get('[data-test-subj="mapping-visual-editor-2-field-type"]')
        .should('have.attr', 'title', 'text')
        .end();
