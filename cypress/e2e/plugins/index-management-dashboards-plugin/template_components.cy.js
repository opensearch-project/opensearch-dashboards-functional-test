...
describe('Component templates', () => {
  before(() => {
    ...
    cy.deleteTemplateComponents(
      `${SAMPLE_TEMPLATE_PREFIX}-${MAX_TEMPLATE_NUMBER}`
    );
    ...
    cy.createIndexTemplate(associatedTemplate, {
      ...
      priority: 100,
      composed_of: [
        `${SAMPLE_TEMPLATE_PREFIX}-0`,
        `${SAMPLE_TEMPLATE_PREFIX}-1`,
      ],
    });
  });
  ...
  describe('can create a component template', () => {
    it('successfully', () => {
      ...
      cy.get('[data-test-subj="form-row-name"] input').type(
        `${SAMPLE_TEMPLATE_PREFIX}-${MAX_TEMPLATE_NUMBER}`
      );
      cy.get('[data-test-subj="CreateComposableTemplateCreateButton"]').click();

      cy.contains(
        `${SAMPLE_TEMPLATE_PREFIX}-${MAX_TEMPLATE_NUMBER} has been successfully created.`
      );
  ...
  describe('can update a component template', () => {
    it('successfully', () => {
      ...
      cy.contains(`${SAMPLE_TEMPLATE_PREFIX}-0`);
      cy.get(
        `[data-test-subj="templateDetail-${SAMPLE_TEMPLATE_PREFIX}-0"]`
      ).click();
      cy.contains('Define component template');
      cy.get('[data-test-subj="form-row-_meta.description"] input').type(
        'Some description'
      );
  ...
  describe('can delete a component template', () => {
    it('successfully', () => {
      ...
      cy.contains(
        /The component template will be unlinked from 1 index templates/
      );
      ...
      cy.wait(500);
      cy.get(`#_selection_column_${SAMPLE_TEMPLATE_PREFIX}-0-checkbox`).should(
        'not.exist'
      );
    });
  });
