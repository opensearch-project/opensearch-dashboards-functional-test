...
describe('Reindex', () => {
  ...
  describe('Reindex validation error', () => {
    ...
    it('source validation failed', () => {
      ...
      cy.get(
        `[data-test-subj="checkboxSelectRow-${REINDEX_DEST_NO_SOURCE}"]`
      ).check({ force: true });

      // Click actions button
      cy.get('[data-test-subj="moreAction"]').click();
      // Reindex should show as activate
      cy.get('[data-test-subj="Reindex Action"]')
        .should('exist')
  ...
  describe('Reindex successfully', () => {
    ...
    it('successfully', () => {
      ...
      cy.get('[data-test-subj="queryJsonEditor"] textarea')
        .focus()
        .clear()
        .type('{"query":{"match":{"category":"Men\'s Clothing"}}}', {
      ...
      // input pipeline
      cy.get(`div[data-test-subj="pipelineCombobox"]`)
        .find(`input[data-test-subj="comboBoxSearchInput"]`)
        .type('bumpOrderId{enter}');
  ...
  describe('Reindex successfully for newly created index', () => {
    ...
    it('successfully', () => {
      ...
      // select logs index
      cy.get(
        '#_selection_column_opensearch_dashboards_sample_data_logs-checkbox'
      ).click();
      ...
      cy.get('[data-test-subj="queryJsonEditor"] textarea')
        .focus()
        .clear()
        .type('{"query":{"match":{"ip":"135.201.60.64"}}}', {
          parseSpecialCharSequences: false,
        });
      ...
      cy.get(
        '[data-test-subj="import-settings-opensearch_dashboards_sample_data_logs"]'
      ).click();
