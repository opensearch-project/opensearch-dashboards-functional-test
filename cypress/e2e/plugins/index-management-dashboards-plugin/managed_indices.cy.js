...
describe('Managed indices', () => {
    ...
    it('successfully', () => {
      ...
      cy.getIndexSettings(SAMPLE_INDEX).then((res) => {
        ...
        expect(res.body).to.have.nested.property(
          'sample_index.settings.index.plugins.index_state_management.rollover_alias',
          FIRST_ALIAS
        );
      });
  ...
  describe('can manage data stream indices', () => {
    ...
    it('successfully', () => {
      ...
      cy.get(
        `[data-test-subj="checkboxSelectRow-.ds-logs-redis-000001"]`
      ).check({ force: true });
      cy.get(`[data-test-subj="Edit rollover aliasButton"]`).should(
        'be.disabled'
      );
    });
  });
